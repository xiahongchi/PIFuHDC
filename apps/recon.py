...
from lib.model import HGPIFuNetwNML, HGPIFuMRNet, ResBlkPIFuNet, HGResBlkPIFuNet
...
def gen_mesh_color(res, net, netC, cuda, data, save_path, thresh=0.5, use_octree=True, components=False):
    image_tensor_global = data['img_512'].to(device=cuda)
    image_tensor = data['img'].to(device=cuda)
    calib_tensor = data['calib'].to(device=cuda)
    ...
    netC.attach(net.netG.get_im_feat())
...
def reconColor(opt, use_rect=False):
    # load checkpoints
    state_dict_path = None
    if opt.load_netMR_checkpoint_path is not None:
        state_dict_path = opt.load_netMR_checkpoint_path
    ...
    if opt.load_netC_checkpoint_path is not None:
        state_dict_path_Color = opt.load_netC_checkpoint_path
    ...
    state_dict_Color = None
    if state_dict_path_Color is not None and os.path.exists(state_dict_path_Color):
        print('Resuming from ', state_dict_path_Color)
        state_dict_Color = torch.load(state_dict_path_Color, map_location=cuda)    
        print('Warning: opt_netC is overwritten.')
        opt_netC = state_dict_Color['opt']
    else:
        print(str(state_dict_path_Color is not None))
        print(str(os.path.exists(state_dict_path_Color)))
        raise Exception('failed loading color state dict!', state_dict_path_Color)
    ...
    netG = HGPIFuNetwNML(opt_netG, projection_mode).to(device=cuda)
    netMR = HGPIFuMRNet(opt, netG, projection_mode).to(device=cuda)
    netC = ResBlkPIFuNet(opt_netC, projection_mode).to(device=cuda)
    ...
    # load checkpoints
    netMR.load_state_dict(state_dict['model_state_dict'])
    netC.load_state_dict(state_dict_Color['model_state_dict'])
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(start_id, end_id)):
            ...
            if True:
                ...
                gen_mesh_color(opt.resolution, netMR, netC, cuda, test_data, save_path, components=opt.use_compose)
            else:
                for j in range(test_dataset.get_n_person(i)):
                    test_dataset.person_id = j
                    test_data = test_dataset[i]
...
def gen_mesh_colorHD(res, net, netC, cuda, data, save_path, thresh=0.5, use_octree=True, components=False):
    image_tensor_global = data['img_512'].to(device=cuda)
    image_tensor = data['img'].to(device=cuda)
    calib_tensor = data['calib'].to(device=cuda)
...
def reconColorHD(opt, use_rect=False):
    # load checkpoints
    state_dict_path = None
    if opt.load_netMR_checkpoint_path is not None:
        state_dict_path = opt.load_netMR_checkpoint_path
    ...
    netG = HGPIFuNetwNML(opt_netG, projection_mode).to(device=cuda)
    netMR = HGPIFuMRNet(opt, netG, projection_mode).to(device=cuda)
    netC = HGResBlkPIFuNet(opt_netC, projection_mode).to(device=cuda)
    ...
    # load checkpoints
    netMR.load_state_dict(state_dict['model_state_dict'])
    netC.load_state_dict(state_dict_Color['model_state_dict'], strict=False)
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(start_id, end_id)):
            ...
            if True:
                ...
                s_model['opt'] = opt_netC
                torch.save(s_model, 'checkpoints/net_C_latest.pt')
                
                
                gen_mesh_colorHD(opt.resolution, netMR, netC, cuda, test_data, save_path, components=opt.use_compose)
            else:
                for j in range(test_dataset.get_n_person(i)):
                    test_dataset.person_id = j
                    test_data = test_dataset[i]
...
def reconColorWrapper(args=None, use_rect=False):
    opt = parser.parse(args)
    reconColor(opt, use_rect)
    
def reconColorHDWrapper(args=None, use_rect=False):
    opt = parser.parse(args)
    reconColorHD(opt, use_rect)
