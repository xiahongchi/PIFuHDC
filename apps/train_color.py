...
def train_color(opt):
    ...
    # create net
    # netG = HGPIFuMRNet(opt, projection_mode).to(device=cuda)
    # load checkpoints
    ...
    if state_dict_path is not None and os.path.exists(state_dict_path):
        ...
        MR_opt = state_dict['opt']
        MR_opt.dataroot = dataroot
        MR_opt.resolution = resolution
        MR_opt.results_path = results_path
        MR_opt.loadSize = loadSize
    else:
    ...
    netMR = HGPIFuMRNet(MR_opt, netG, projection_mode).to(device=cuda)
    ...
    #     if 'hgcater.' not in name:
    #         param.requires_grad = False
    opt.learning_rate = opt.learning_rate * (opt.batch_size / 32)
    ...
    print('Using NetworkG: ', netG.name, '  ', netMR.name, 'networkC: ', netC.name)
    ...
    opt.checkpoints_path = './color_train_models'
    opt.name = 'train_all_netC'
    ...
    for epoch in range(start_epoch, opt.num_epoch):
        ...
        for train_idx, train_data in enumerate(train_data_loader):
            ...
            # retrieve the data
            image_tensor_512 = train_data['img'].to(device=cuda)
            image_tensor = train_data['img-norz'].to(device=cuda)
            calib_tensor = train_data['calib'].to(device=cuda)
            color_sample_tensor = train_data['color_samples'].to(device=cuda)

            image_tensor_512, image_tensor, calib_tensor = reshape_multiview_tensors(image_tensor_512, image_tensor, calib_tensor)
            ...
            with torch.no_grad():
                netMR.filter_global(image_tensor_512)
                netMR.filter_local(image_tensor[:,None])
            resC, error = netC.forward(image_tensor, netMR.get_im_feat(), color_sample_tensor, calib_tensor, labels=rgb_tensor)
