...
class TrainDataset(Dataset):
    ...
    def __init__(self, opt, phase='train'):
        ...
        self.to_tensor_noresize = transforms.Compose([
    ...
    def get_render(self, subject, num_views, yid=0, pid=0, random_sample=False):
        ...
        for vid in view_ids:
            ...
            render_resize = render.resize((512, 512))
            render_resize = self.to_tensor(render_resize)
            render_noresize = self.to_tensor_noresize(render)
            # render = mask.expand_as(render) * render

            render_list.append(render_resize)
            render_noresize_list.append(render_noresize)
            calib_list.append(calib)
            extrinsic_list.append(extrinsic)

        return {
            'img': torch.stack(render_list, dim=0),
            'img-norz': torch.stack(render_noresize_list, dim=0),
